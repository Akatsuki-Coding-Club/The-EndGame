          ...
          {podium.length > 0 && (
            <div className="flex flex-col md:flex-row items-end justify-center gap-4 md:gap-6 mt-8 md:mt-12 px-4">
              {podium.map((team) => {
                ...
                return (
                  <div key={team.teamId} className={cn(
              ...
              {mergedLogs.length === 0 ? (
                ...
                mergedLogs.map((notif) => (
                  <div
                    key={notif.id}
                    className={cn(
                      "p-3.5 rounded-xl text-[10px] font-mono animate-in slide-in-from-right-4 duration-500 flex flex-col gap-2 relative overflow-hidden group border",
