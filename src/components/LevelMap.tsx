...
const LevelMap = ({ onSelectLevel }: Props) => {
  ...
  return (
    <div className="animate-fade-in">
      <h2 className="font-sans font-bold text-2xl text-primary neon-text mb-6 tracking-wider">LEVEL MAP</h2>
        ...
        {puzzles.map(puzzle => {
          ...
          return (
            <button
              ...
              disabled={!isUnlocked}
              className={`glass-card p-5 text-left transition-all duration-300 group ${isCompleted
            ...
            >
              ...
              <h3 className={`font-sans font-bold text-lg mb-1 ${isCompleted ? "text-accent" : isCurrent ? "text-primary" : "text-foreground"
                }`}>
                {puzzle.title}
              </h3>
              <p className="text-xs font-sans text-muted-foreground">{puzzle.description}</p>
              <div className="mt-3 flex items-center justify-between">
                <span className="text-xs font-sans text-muted-foreground">{puzzle.points} pts</span>
                {isCurrent && !isCompleted && (
                  <span className="text-xs font-sans text-primary animate-neon-pulse uppercase tracking-wider">Current</span>
                )}
              </div>
            </button>
          );
